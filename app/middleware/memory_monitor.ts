...
export default class MemoryMonitorMiddleware {
  ...
  private logMemoryUsage() {
    ...
    if (memoryUsage.heapUsed > 1024 * 1024 * 1024) { // > 1GB heap
    ...
    if (global.gc) {
      ...
      console.log(`Heap Used: ${formatMemory(afterGC.heapUsed)} MB (Freed: ${formatMemory(memoryUsage.heapUsed - afterGC.heapUsed)} MB)`)
    }
  }
} 