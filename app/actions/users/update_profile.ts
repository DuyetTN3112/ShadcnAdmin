  ...
  try {
    ...
    await user
      .merge({
        ...
        email: data.email,
        password: data.new_password ? await hash.make(data.new_password) : user.password,
      })
      .save()

    // Cập nhật thông tin chi tiết
    if (Object.keys(detailData).length > 0) {
      const userDetail = await UserDetail.firstOrNew({ user_id: userId }, { user_id: userId })
    ...
    if (Object.keys(profileData).length > 0) {
      const userProfile = await UserProfile.firstOrNew({ user_id: userId }, { user_id: userId })
    ...
    if (urls && urls.length > 0) {
      // Xóa URLs cũ
      await UserUrl.query().where('user_id', userId).delete()
      // Thêm URLs mới
      for (const url of urls) {
        await UserUrl.create({ user_id: userId, url })
      }
    }
