...
export default class ListConversations {
  ...
  async handle(options: FilterOptions = {}): Promise<PaginatedResponse<Conversation>> {
    ...
    const mainQuery = Conversation.query()
      .whereHas('conversationParticipants', (participantBuilder) => {
        participantBuilder.where('userId', user.id)
      })
      .whereNull('deletedAt')
      // Preload participants với user data
      .preload('participants')
      .preload('conversationParticipants', (participantQuery) => {
      ...
      mainQuery.where((searchBuilder) => {
        searchBuilder
          ...
          .orWhereHas('participants', (userSearchBuilder) => {
            userSearchBuilder.where('fullName', 'LIKE', `%${options.search}%`)
          })
      })
    }
    // Sắp xếp theo tin nhắn mới nhất
    mainQuery.orderBy('updatedAt', 'desc')
